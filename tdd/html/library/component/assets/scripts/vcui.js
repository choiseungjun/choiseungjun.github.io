...
;(function (global, undefined) {
    ...
    if (!('importBasePath' in _configs)) {
        (function () {    // import 할 baseUrl이 설정안돼있을 경우 vcui.js경로를 알아내 설정해준다.
            ...
            for (var i = -1, item; item = scripts[++i];) {
                if (item.src && (m = item.src.match(/(.*)vcui\.(min\.)?js.*/i))) {
                    _configs.importBasePath = m[1];
                    _configs.loadMinify = !!m[2];    // vcui.min.js 으로 임포트돼있으면 ui모듈도 min파일로 임포트되도록 설정
                    return;
                }
            }
        })();
    ...
    // 커스텀 이벤트 생성
    function createCustomWindowEvent() {
        $win.on(function () {
...
;(function ($, core, global, undefined) {
    ...
    var isA = function (a, b) {
        ...
        loader = {
            urlParse: function (pUrl, type) {
                ...
                if(fn.loadMinify) {
                    parts.url = url.replace(/(?!min)\.(js|css)/i, '.min.$1');
                } else {
                    parts.url = url;
                }
    ...
    core.require = function (files, fn) {
        var def = $.Deferred();
        load(files, function () {
            def.resolve();
            fn && fn();
    ...
    //alert(core.detect.version);
    if (core.detect.isIE && core.detect.version >= 8) {
        core['import'] = core.require;
    }
...
;(function ($, core, global, undefined) {
    ...
    core.addon('dom', {
         ...
         * @return {number}
         */
        measureText: function(text, parent){
            if (!text){
                return 0;
            }
...
//
    ...
    var View = core.BaseClass.extend(/** @lends vcui.ui.View# */{
        ...
        docOn: function (eventName) {
            $doc.on.apply($doc, [this.makeEventNS(eventName)].concat([].slice.call(arguments, 1)));
            return this;
        },

        docOff: function (eventName) {
            $doc.off.apply($doc, [eventName ? this.makeEventNS(eventName) : this.getEventNS()].concat([].slice.call(arguments, 1)));
            return this;
        },
        ...
        destroy: function () {
            ...
            self.$el.off(self.eventNS);
            self.$el.removeData('ui_' + self.moduleName);
            $(global).off(self.getEventNS());
            $(document).off(self.getEventNS());
            ...
            core.ui.View._instance = core.array.remove(core.ui.View._instances, self);
        }
    });
