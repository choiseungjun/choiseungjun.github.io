;(function ($, core, global, undefined) {
  ...
  function createBreakpointsEvent() {
    ...
    var breakpoints = core.configs.breakpoints || [{
      mode: "xs",
      min: 0,
      max: 376
    }, {
    ...
    $win.on('resize.breakpoints orientationchange.breakpoints load.breakpoints', function () {
      var w = $win.width();
      for (var i = 0, item; item = breakpoints[i]; i++) {
        if (w > item.min && w <= item.max && prev != item.mode) {
          $win.trigger('breakpoint:change', current = item);
          prev = item.mode;
        }
      }
    });

    $win.on('breakpoint:change', function (e, data) {
      if (!data) {
        $win.trigger('breakpoint:change', current);
      }
    });
  }
